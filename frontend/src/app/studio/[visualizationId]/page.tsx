...
import { CheckCircle2, ChevronDown, Download, Undo2 } from "lucide-react";
...
export default function StudioPage() {
  ...
  return (
    <AppShell>
      <PipelineStepper
        current="studio"
        projectId={visualizationQuery.data?.project_id}
        datasetId={visualizationQuery.data?.dataset_id}
      />
      <section className="mx-auto flex max-w-5xl flex-col gap-6 px-6 py-16">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex shrink-0 gap-2">
            <Button
              variant="outline"
              size="sm"
              disabled={(versionsQuery.data?.length ?? 0) < 2 || undoMutation.isPending}
              onClick={() => undoMutation.mutate()}
            >
              <Undo2 aria-hidden className="mr-1.5 h-4 w-4" />
              {undoMutation.isPending ? "Undoing…" : "Undo"}
            </Button>
            <Button variant="accent" size="sm" onClick={() => setRightTab("export")}>
              <Download aria-hidden className="mr-1.5 h-4 w-4" />
              Export
            </Button>
          </div>
        </div>
        {(exportError || undoMutation.isError) && (
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_340px]">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div
              id="studio-inspector"
              className={cn("flex-col gap-5", inspectorOpen ? "flex" : "hidden", "lg:flex")}
            >
              <Tabs
