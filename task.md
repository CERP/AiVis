...
- [ ] ER diagram + SQLModel models with FKs, indexes, timestamps, soft-delete where relevant
- Acceptance: `ARCHITECTURE.md` includes ERD

### P4-002 — Alembic migrations setup
- [ ] Init alembic, first migration
- Deps: P4-001
- Acceptance: `alembic upgrade head` works against local Postgres

### P4-003 — DB session/connection management (async SQLAlchemy engine)
- [ ] `core/db.py`
- Acceptance: FastAPI dependency yields session

### P4-004 — Repository layer per entity
- [ ] `repositories/*.py` implementing CRUD, isolated from routes
- Deps: P4-002
